...
export default function AdminSearchAnalyticsPage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => <div key={i} className="bg-white rounded-xl border p-5 animate-pulse"><div className="h-8 bg-gray-200 rounded w-16" /></div>)}
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="bg-white rounded-xl border p-12 text-center">
        ...
        <p className="text-sm text-gray-500">Analytics data will appear as users interact with the platform</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Analytics</h1>
        <p className="text-sm text-gray-500 mt-0.5">Platform performance and usage metrics</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-5">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Daily Activity (Last 7 Days)</h3>
          <div className="flex items-end gap-2 h-40">
            {data.dailyActivity.map((day) => (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-xs font-medium text-gray-900">{day.actions}</span>
        ...
        {/* Top Phones */}
        <div className="bg-white rounded-xl border p-5">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Top Viewed Phones</h3>
          {data.topPhones.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-8">No phone views recorded yet</p>
          ) : (
            <div className="space-y-3">
              {data.topPhones.map((phone, i) => (
                <div key={phone.id} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-gray-600">{phone.viewCount.toLocaleString()} views</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Action Breakdown */}
        <div className="bg-white rounded-xl border p-5">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Actions by Type (Last 30 Days)</h3>
          {Object.keys(data.actionBreakdown).length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-8">No actions recorded</p>
          ) : (
            <div className="space-y-3">
        ...
        {/* Entity Breakdown */}
        <div className="bg-white rounded-xl border p-5">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Actions by Entity (Last 30 Days)</h3>
          {Object.keys(data.entityBreakdown).length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-8">No entity data</p>
          ) : (
            <div className="space-y-2">
              {Object.entries(data.entityBreakdown)
                .sort((a, b) => b[1] - a[1])
                .map(([entity, count]) => (
                  <div key={entity} className="flex items-center justify-between py-1.5 px-3 hover:bg-gray-50 rounded">
                    <span className="text-sm font-medium text-gray-700 capitalize">{entity}</span>
