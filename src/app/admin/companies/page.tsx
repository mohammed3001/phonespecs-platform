    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border shadow-sm p-6 space-y-4">
          <h3 className="text-lg font-semibold text-gray-900">{editing ? "Edit Company" : "New Company"}</h3>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))}
              rows={2} className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b">
              ...
              <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase">Verified</th>
          ...
          <tbody className="divide-y">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-500">No companies yet. Click &quot;Add Company&quot; to create one.</td></tr>
            ) : (
              companies.map((company) => (
                <tr key={company.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700 font-medium capitalize">{company.type}</span>
                  </td>
                  <td className="px-4 py-3 text-center text-sm text-gray-600">{company.userCount}</td>
