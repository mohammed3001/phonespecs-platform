...
export default function AdminUsersPage() {
  ...
  const userTypeColors: Record<string, string> = {
    admin: "bg-red-50 dark:bg-red-900/20 text-red-700",
    moderator: "bg-purple-50 dark:bg-purple-900/20 text-purple-700",
    company: "bg-blue-50 dark:bg-blue-900/20 text-blue-700",
    user: "bg-gray-100 text-gray-700 dark:text-gray-200",
  };
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl border shadow-sm dark:shadow-gray-900/30 p-6 space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{editing ? "Edit User" : "New User"}</h3>
          {error && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{editing ? "New Password (leave blank to keep)" : "Password *"}</label>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-900 border-b">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Last Login</th>
                ...
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-blue-600 text-sm font-bold">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${userTypeColors[user.userType] || "bg-gray-100 text-gray-700 dark:text-gray-200"}`}>
                        {user.userType}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{user.role?.name || "—"}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{user.company?.name || "—"}</td>
