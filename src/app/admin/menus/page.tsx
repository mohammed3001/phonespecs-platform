...
export default function AdminMenusPage() {
  ...
  const renderItem = (item: MenuItem, menuId: string, depth: number = 0) => (
    <div key={item.id}>
      <div className={`flex items-center justify-between py-2 px-3 hover:bg-gray-50 rounded`} style={{ paddingLeft: `${12 + depth * 24}px` }}>
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => startEditItem(item, menuId)} className="p-1 hover:bg-gray-200 rounded text-gray-500">
            <Icon icon="mdi:pencil-outline" width={14} />
          </button>
          <button onClick={() => handleDeleteItem(item.id)} className="p-1 hover:bg-red-100 rounded text-red-400">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{menus.length} menus</p>
        </div>
      ...
      {showAddMenu && (
        <form onSubmit={handleCreateMenu} className="bg-white rounded-xl border p-5 space-y-4">
          <h3 className="text-sm font-semibold text-gray-900">New Menu</h3>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowAddMenu(false)} className="text-gray-600 px-4 py-2 text-sm hover:bg-gray-100 rounded-lg">Cancel</button>
          </div>
        </form>
      )}

      {loading ? (
        <div className="space-y-4">{[...Array(3)].map((_, i) => <div key={i} className="bg-white rounded-xl border p-6 animate-pulse"><div className="h-5 bg-gray-200 rounded w-40" /></div>)}</div>
      ) : menus.length === 0 ? (
        <div className="bg-white rounded-xl border p-12 text-center">
          ...
          <p className="text-sm text-gray-500">Create menus to manage your site navigation</p>
        </div>
      ) : (
        menus.map((menu) => (
          <div key={menu.id} className="bg-white rounded-xl border overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b bg-gray-50">
              <button onClick={() => setExpandedMenu(expandedMenu === menu.id ? null : menu.id)} className="flex items-center gap-3 flex-1 text-left">
                <Icon icon="mdi:menu" width={20} className="text-gray-500" />
                <div>
                  <h3 className="font-semibold text-gray-900">{menu.name}</h3>
                  <p className="text-xs text-gray-500">{menu.items.length} items {menu.location && `• ${menu.location}`}</p>
                </div>
              </button>
              <div className="flex items-center gap-2">
                <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded-full">{menu.slug}</span>
                <button onClick={() => handleDeleteMenu(menu.id, menu.name)} className="p-1.5 hover:bg-red-100 rounded text-red-400">
            ...
            {(expandedMenu === menu.id || true) && (
              <div className="p-4">
                ...
                {showAddItem === menu.id && (
                  <form onSubmit={handleSaveItem} className="bg-gray-50 rounded-lg p-4 mb-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={() => { setShowAddItem(null); setEditItem(null); }} className="text-gray-600 px-3 py-1.5 text-sm hover:bg-gray-200 rounded-lg">Cancel</button>
                    </div>
                  </form>
                )}

                {menu.items.length === 0 ? (
                  <p className="text-sm text-gray-500 text-center py-4">No items in this menu</p>
                ) : (
