...
export default function AdminSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showAdd && (
        <form onSubmit={handleAddSetting} className="bg-white rounded-xl border p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input required value={newSetting.key} onChange={(e) => setNewSetting((p) => ({ ...p, key: e.target.value }))}
                className="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="site_name" />
            </div>
            <div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowAdd(false)} className="text-gray-600 px-4 py-2 text-sm hover:bg-gray-100 rounded-lg">Cancel</button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No settings configured</h3>
          <p className="text-sm text-gray-500">Click &quot;Add Setting&quot; to create your first configuration.</p>
        </div>
      ) : (
        Object.entries(grouped).map(([group, items]) => (
          <div key={group} className="bg-white rounded-xl border">
            <div className="px-5 py-4 border-b bg-gray-50 rounded-t-xl">
              <h3 className="text-sm font-semibold text-gray-900 capitalize">{group}</h3>
            </div>
